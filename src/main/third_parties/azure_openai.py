...
class AzureOpenAI(third_party.ThirdPartyBase):
    ...
    def get_apikeey_slot_info_list(self) -> list[third_party.ApiKeeySlotInfo]:
        """Gets the API key slot information required for Azure OpenAI.

        Returns:
            list[third_party.ApiKeeySlotInfo]: A list containing information
                about the 'azure_openai' API key slot.
        """
        return [
            third_party.ApiKeeySlotInfo(
                apikeey_slot_id="azure_openai",
                name="Azure OpenAI"
            )
        ]
    ...
    def get_aiengine_info_list(self) -> list[third_party.AIEngineInfo]:
        ...
        return [
            third_party.AIEngineInfo(
                ...
                name="Azure OpenAI",
                apikeey_slot_id_list=["azure_openai"],
                arg_list=[
    ...
    def generate_response(
        ...
        Args:
            ...
            aiengine_arg_dict (dict[str, str]): Arguments for the AI engine,
                ...
                'system_prompt'.
            apikeey_list (list[str]): List containing the Azure OpenAI API key.
        ...
        assert (len(apikeey_list) == 1), "Azure OpenAI requires exactly one API key."
        assert (apikeey_list[0] is not None), "Azure OpenAI API key cannot be None."

        deployment_name = aiengine_arg_dict["deployment_name"]
        system_prompt = aiengine_arg_dict.get("system_prompt", "")
        apikeey = apikeey_list[0]
    ...
    def _get_client(self, apikeey: str, azure_endpoint: str, api_version: str) -> openai.AzureOpenAI:
        """Retrieves or creates an AzureOpenAI client.

        Manages a dictionary of client instances, keyed by a tuple of
        (apikeey, azure_endpoint, api_version), to reuse clients if possible.

        Args:
            apikeey (str): The Azure OpenAI API key.
        ...
        """
        client_key = (apikeey, azure_endpoint, api_version)
