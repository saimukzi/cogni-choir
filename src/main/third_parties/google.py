...
class Google(third_party.ThirdPartyBase):
    ...
    def get_aiengine_info_list(self) -> list[third_party.AIEngineInfo]:
        ...
        return [
            third_party.AIEngineInfo(
                ...
                name="Google Gemini",
                apikeey_slot_id_list=["google_gemini"],
                arg_list=[
    ...
    def generate_response(
        ...
        Args:
            ...
            apikeey_list (list[str]): List containing the Google API key.
        ...
        assert (len(apikeey_list) == 1), "Requires exactly one API key."
        ...
        client = self._get_client(apikeey)
    ...
    def _get_client(self, apikeey: str) -> genai.Client:
        ...
        Args:
            apikeey (str): The Google API key.

        Returns:
            genai.Client: An initialized Google GenAI client instance.
        """
        if apikeey not in self._apikeey_to_client_dict:
            client = genai.Client(api_key=apikeey)
            self._apikeey_to_client_dict[apikeey] = client
        return self._apikeey_to_client_dict[apikeey]
