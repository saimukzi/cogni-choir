...
class TestBot(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # thirdpartyapikey_query_list is initialized to None by Bot.__init__
    ...
    def test_bot_creation(self): # Ensure basic attributes are set
        ...
        self.assertListEqual(self.bot.thirdpartyapikey_query_list, []) # Ensure it's an empty list if not set
...
class TestGoogleEngine(unittest.TestCase): # Renamed from TestGeminiEngine
    ...
    def test_google_init_success(self, mock_genai_sdk): # Renamed
        ...
        # Google class __init__ takes no thirdpartyapikey or model_name. These are passed to generate_response.
        engine = Google()
        ...
        self.assertEqual(len(engine._thirdpartyapikey_to_client_dict), 0)
    ...
    @patch('src.main.third_parties.google.genai') # Patched to new location
    def test_google_no_thirdpartyapikey(self, mock_genai_sdk): # Renamed
        """Tests Google engine's behavior when an API key is not provided in list."""
        engine = Google()
        aiengine_arg_dict = {"model_name": "gemini-no-key"}
        # generate_response in Google class asserts `len(thirdpartyapikey_list) == 1`.
...
class TestAzureOpenAIEngine(unittest.TestCase):
    ...
    def test_openai_init_and_client_retrieval(self, mock_sdk_azure_openai_class):
        ...
        mock_sdk_azure_openai_class.assert_called_once_with(
            api_key=thirdpartyapikey_list[0],
        ...
        # self.assertIn((thirdpartyapikey_list[0], "fake_endpoint_init", '2024-12-01-preview'), engine._client_dict)
        # self.assertEqual(engine._client_dict[(thirdpartyapikey_list[0], "fake_endpoint_init", '2024-12-01-preview')], self.mock_openai_client_instance)
    ...
    def test_openai_generate_response_success(self, mock_sdk_azure_openai_class):
        ...
        self.mock_openai_client_instance.chat.completions.create.assert_called_once_with(
            model=aiengine_arg_dict_for_test["deployment_name"], # Corrected key
            messages=expected_api_messages
        )
        mock_sdk_azure_openai_class.assert_called_once_with(
            api_key=thirdpartyapikey_list_for_test[0],
    ...
    def test_openai_no_thirdpartyapikey_or_improper_config(self):
        """
        Tests AzureOpenAI's behavior when no API key is provided or if the configuration is improper.
        """
        engine = AzureOpenAI()
        aiengine_arg_dict = {"deployment_name": "deployment-no-key", "system_prompt": ""} # Corrected key

        with patch('src.main.third_parties.azure_openai.commons.read_str', return_value="fake_endpoint_no_key"):
            # Test with empty thirdpartyapikey list (AssertionError)
...
class TestXAIEngine(unittest.TestCase): # Renamed from TestGrokEngine
    ...
    def test_xai_init_no_thirdpartyapikey(self): # Renamed
        """XAI class takes no thirdpartyapikey in __init__. This test might be obsolete or test generate_response."""
        # This test as-is for __init__ is not applicable.
        # Testing no thirdpartyapikey for generate_response:
        engine = XAI()
        aiengine_arg_dict = {"model_name": "grok-no-key"}
        with self.assertRaises(AssertionError): # XAI asserts len(thirdpartyapikey_list) == 1
    ...
    def test_xai_response_api_connection_error(self, mock_openai_constructor, mock_logging_error):
        ...
        thirdpartyapikey_list = ["fake_xai_key_conn_error"]
        response = engine.generate_response("xai_grok", aiengine_arg_dict, thirdpartyapikey_list, "TestRole", [])
    ...
    def test_xai_response_rate_limit_error(self, mock_openai_constructor, mock_logging_error):
        ...
        thirdpartyapikey_list = ["fake_xai_key_rate_limit"]
        response = engine.generate_response("xai_grok", aiengine_arg_dict, thirdpartyapikey_list, "TestRole", [])
    ...
    def test_xai_response_authentication_error(self, mock_openai_constructor, mock_logging_error):
        ...
        thirdpartyapikey_list = ["fake_xai_key_auth_error"]
        response = engine.generate_response("xai_grok", aiengine_arg_dict, thirdpartyapikey_list, "TestRole", [])
    ...
    def test_xai_response_api_error(self, mock_openai_constructor, mock_logging_error):
        ...
        thirdpartyapikey_list = ["fake_xai_key_api_error"]
        response = engine.generate_response("xai_grok", aiengine_arg_dict, thirdpartyapikey_list, "TestRole", [])
    ...
    def test_xai_response_unexpected_exception(self, mock_openai_constructor, mock_logging_error):
        ...
        thirdpartyapikey_list = ["fake_xai_key_unexpected_error"]
        response = engine.generate_response("xai_grok", aiengine_arg_dict, thirdpartyapikey_list, "TestRole", [])
    ...
    # For XAI (which inherits ThirdPartyBase), the thirdpartyapikey and model_name are not direct __init__ args.
    ...
    #         # Attributes like thirdpartyapikey, model_name are not stored on XAI instance from __init__
    #         # self.assertEqual(engine.thirdpartyapikey, "key123")
...
#         # engine_config = {"engine_type": "Google", "thirdpartyapikey": "test_google_key"}
...
#         # self.assertEqual(bot.get_engine().thirdpartyapikey, "test_google_key") # If thirdpartyapikey is stored on engine
#         pass

#     def test_create_azure_openai_bot_success(self): # Renamed from test_create_openai_bot_success
#         # engine_config = {"engine_type": "AzureOpenAI", "thirdpartyapikey": "test_openai_key"}
...
#         # self.assertEqual(bot.get_engine().thirdpartyapikey, "test_openai_key")
#         pass

#     @patch('src.main.third_parties.xai.openai.OpenAI') # Example
#     def test_create_xai_bot_success(self, mock_xai_sdk): # Renamed
#         # engine_config = {"engine_type": "XAI", "thirdpartyapikey": "test_xai_key"}
...
#         # self.assertEqual(bot.get_engine().thirdpartyapikey, "test_xai_key")
#         pass
#
...
#        # engine_config = {"engine_type": "Google", "thirdpartyapikey": "test_key_default_model"}
