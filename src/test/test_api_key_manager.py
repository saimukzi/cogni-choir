"""Unit tests for the ApiKeeyManager class with encryption."""
import unittest
...
from main.apikeey_manager import ApiKeeyManager, ApiKeeyQuery, ENCRYPTED_SERVICE_NAME_PREFIX # Removed _KEYRING_MANAGED_SERVICES_KEY, Added ApiKeeyQuery
...
TEST_API_KEYS_FILE = os.path.join(DATA_DIR, "test_apikeeys.json")
TEST_ENCRYPTION_SALT_FILE_FOR_AKM = os.path.join(DATA_DIR, "test_akm_encryption_salt.json")


class TestApiKeeyManagerWithEncryption(unittest.TestCase):
    """Tests for the ApiKeeyManager class, assuming keyring usage with encryption."""
    ...
    def setUp(self):
        ...
        # ApiKeeyManager now requires encryption_service and data_path.
        ...
        with patch('keyring.get_password') as mock_init_keyring_get:
            ...
            self.api_manager = ApiKeeyManager(
    ...
    def test_initial_state(self):
        ...
        self.assertEqual(self.api_manager._data, {'apikeey_slot_id_to_apikeey_id_list_dict': {}}, "Initial data should be empty manifest.")
    ...
    def test_save_load_key_encrypted(self, mock_keyring_get_password, mock_keyring_set_password):
        ...
        slot_id = "TestSlot"
        key_id = "TestKeyID"
        apikeey = "secret_key_123"
        api_query = ApiKeeyQuery(slot_id, key_id)

        # Simulate key not found initially for get_apikeey if it tries to read before setting mock
        mock_keyring_get_password.return_value = None

        # Action: Save key
        self.api_manager.set_apikeey(api_query, apikeey)
        ...
        self.assertNotEqual(encrypted_key_arg, apikeey) # Ensure it's not plaintext
        ...
        # Action: Load key
        loaded_key = self.api_manager.get_apikeey(api_query)
        self.assertEqual(loaded_key, apikeey, "Loaded key should match original after decryption.")
        mock_keyring_get_password.assert_called_with(expected_keyring_service, key_id) # Called by get_apikeey

        # Verify manifest file
        self.assertTrue(os.path.exists(TEST_API_KEYS_FILE))
        with open(TEST_API_KEYS_FILE, 'r') as f:
            manifest_data = json.load(f)
        self.assertIn(slot_id, manifest_data['apikeey_slot_id_to_apikeey_id_list_dict'])
        self.assertIn(key_id, manifest_data['apikeey_slot_id_to_apikeey_id_list_dict'][slot_id])
    ...
    def test_delete_key_encrypted(self, mock_get_password, mock_set_password, mock_delete_password):
        ...
        apikeey = "key_to_delete"
        api_query = ApiKeeyQuery(slot_id, key_id)

        # Save a key first
        encrypted_key_placeholder = self.encryption_service.encrypt(apikeey)
        ...
        self.assertIsNotNone(self.api_manager.get_apikeey(api_query), "Key should be loadable before delete.")
        
        # Action: Delete key
        self.api_manager.delete_apikeey(api_query)
        ...
        mock_get_password.return_value = None # Ensure get_apikeey now finds nothing
        self.assertIsNone(self.api_manager.get_apikeey(api_query), "Key should be None after delete.")

        # Verify manifest file updated
        with open(TEST_API_KEYS_FILE, 'r') as f:
            manifest_data = json.load(f)
        self.assertNotIn(key_id, manifest_data['apikeey_slot_id_to_apikeey_id_list_dict'].get(slot_id, []),
    ...
    def test_load_key_decryption_failure_keyring(self, mock_keyring_get_password):
        ...
        # Pre-populate manifest so that get_apikeey attempts to load this
        self.api_manager._data['apikeey_slot_id_to_apikeey_id_list_dict'][slot_id] = [key_id]
        # No need to save manifest here as get_apikeey doesn't write it
    ...
    def test_re_encrypt_all_keys_keyring(self, mock_keyring_set_password, mock_keyring_get_password):
        ...
        query1, query2 = ApiKeeyQuery(slot1, id1), ApiKeeyQuery(slot2, id2)
        ...
        # Populate manifest
        self.api_manager._data['apikeey_slot_id_to_apikeey_id_list_dict'] = {
            slot1: [id1],
            slot2: [id2]
        }
        ...
        # Action: Re-encrypt. This will use the new ApiKeeyManager.re_encrypt logic
    ...
    def test_clear_all_keys_and_data_keyring(self, mock_keyring_delete_password):
        ...
        self.assertTrue(os.path.exists(TEST_API_KEYS_FILE))
        # Salt file is managed by EncryptionService, ApiKeeyManager.clear() does not directly delete it.
        ...
        # Data file should be removed by ApiKeeyManager.clear()
        self.assertFalse(os.path.exists(TEST_API_KEYS_FILE), "Data file (manifest) should be deleted.")

        # Check internal state
        self.assertEqual(self.api_manager._data, {'apikeey_slot_id_to_apikeey_id_list_dict': {}},
                         "Internal data should be reset to empty manifest after clear.")


    def test_set_apikeey_requires_encryption_service(self):
        """Tests that set_apikeey raises RuntimeError if encryption_service is (somehow) None."""
        # ApiKeeyManager __init__ now requires encryption_service.
        ...
        with self.assertRaisesRegex(RuntimeError, "Encryption service not available"):
            self.api_manager.set_apikeey(ApiKeeyQuery("Test", "TestID"), "key")
        self.api_manager.encryption_service = original_es # Restore

    def test_get_apikeey_requires_encryption_service(self):
        """Tests that get_apikeey raises RuntimeError if encryption_service is (somehow) None."""
        original_es = self.api_manager.encryption_service
        self.api_manager.encryption_service = None
        with self.assertRaisesRegex(RuntimeError, "Encryption service not available"):
            self.api_manager.get_apikeey(ApiKeeyQuery("Test", "TestID"))
        self.api_manager.encryption_service = original_es # Restore

    def test_empty_service_or_key_with_encryption(self):
        """Tests handling of empty ApiKeeyQuery fields with encryption enabled."""
        # ApiKeeyManager.set_apikeey now takes ApiKeeyQuery.
        # The ValueError "Service name and API key cannot be empty" was for key string.
        # ApiKeeyQuery itself doesn't prevent empty strings for slot_id/apikeey_id in constructor.
        # The actual keyring.set_password might fail with empty username/password.
        # Let's test that ApiKeeyManager.set_apikeey raises ValueError for empty apikeey string.

        with self.assertRaisesRegex(ValueError, "API key cannot be empty"): # Assuming SUT checks this for apikeey
            self.api_manager.set_apikeey(ApiKeeyQuery("some_slot", "some_id"), "")

        # Test with empty slot_id or key_id in ApiKeeyQuery - depends on how SUT handles this for keyring.
        # Keyring might allow empty service/username but it's bad practice.
        # For now, assume ApiKeeyQuery itself can be created with empty strings.
        # The SUT's set_apikeey has `if not apikeey_query: raise ValueError(...)`
        # but this checks if the ApiKeeyQuery object itself is None, not its content.
        # The original test was `self.api_manager.set_apikeey("", "some_key")`.
        # Let's assume an ApiKeeyQuery with empty parts is passed.
        # The ValueError for empty apikeey string is the main part to keep.

        # What about get_apikeey with empty ApiKeeyQuery fields?
        # `if not apikeey_query: raise ValueError(...)`
        with self.assertRaisesRegex(ValueError, "ApiKeeyQuery object cannot be None."): # Updated expected message
            self.api_manager.get_apikeey(None) # Test None query
        ...
        with patch('keyring.get_password') as mock_keyring_get_empty:
            ...
            self.assertIsNone(self.api_manager.get_apikeey(ApiKeeyQuery("", "")),
